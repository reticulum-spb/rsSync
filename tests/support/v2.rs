...
use rrsync::{
    ...
    config::Permission,
    engine::{
        ConnectionId, ReceivedFile, SyncTransport, TransferId,
        v2::{Cache, Server, ServerReply},
    },
    fs::Root,
...
impl SimulatedTransport {
    ...
    fn dispatch(&mut self, payload: &[u8]) -> Result<Vec<u8>> {
        ...
        let outcome = Message::decode(payload).and_then(|message| {
            server.handle(self.connection, self.peer, self.permission, message)
        });
        let response = match outcome {
            ...
            Err(error) => {
                ...
                Message::Common(Common::error(&error)).encode()
            }
        };
        if response.is_err() {
            server.disconnect(self.connection);
    ...
    async fn request_inner(&mut self, payload: Vec<u8>, deadline: Duration) -> Result<Vec<u8>> {
        ...
        if tag == 17 && self.take_fault(Fault::FinishBeforeCommit) {
            return self.dispatch(&Message::Common(Common::Finish).encode()?);
        }
...
impl SyncTransport for SimulatedTransport {
    ...
    async fn send_file(
        ...
        if self.take_fault(Fault::RestartServerAfterUpload(index)) {
            *self.server.lock().unwrap() = Server::new(self.root.clone(), self.cache.clone());
            return Err(lost());
        }
        Ok(transfer_id)
    }
